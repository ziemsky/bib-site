...
Podane procedury skupiają się na użyciu myszki, ograniczając użycie klawiatury do koniecznego minimum. Powodem jest to aby uczynić naukę podstaw QCAD'a jak najłatwiejszą, kosztem szybkości rysowania.
...
Niektóre z członów ścieżki mogą się odnosić do tekstowych menu, a inne do opcji lub pól w oknach dialogowych. Powyższa, przykładowa ścieżka pozwala na znalezienie pola do ustawienia systemu miar. Jest ona zilustrowana na zrzucie ekranu poniżej; zauważ jak elementy ścieżki korespondują do elementów zaznaczonych na poniższej ilustracji:
...
`LMB2`:: Dwukrotne kliknięcie lewego klawisza myszy.
`RMB`:: Pojedyncze kliknięcie prawego klawisza myszy (_Right Mouse Button_).

==== Panele
Gdy przewodnik mówi o _Block List_,  _Layer List_ lub _Property Editor_, to odnosi się do paneli po prawej stronie ekranu.
...
Gdy te opcje są zaznaczone, korespondujące panele powinny być widoczne po prawej stronie ekranu, podobnie jak na poniższych ilustracjach:
...
=== Rozmieść detale
Rysując model nie przejmujemy się detalami rozmieszczenia elementów na rysunku, ale odrobina planowania jest potrzebna, jako że znacznie ułatwi nam ona przygotowanie wydruku później.

Na przykład, rysując konstrukcję stalową zwykle potrzebujemy przedstawić widok (lub widoki) elementu wysyłkowego oraz, jako osobne widoki, pojedyncze detale jakie się na ten widok składają. Rysując takie elementy w modelu wyobraź sobie, że każdy z nich jest obrysowany jego własną, prostokątną granicą. Następnie, rozmieść je tak aby te granice się nie nakładały.

Porównaj następujące dwa przykłady; szara, przerywana linia ilustruje wyobrażone "granice" każdego z elementów:

.Niepoprawne rozmieszczenie elementów w modelu - granice się przecinają
image::img/model_rozmieszczenie_detali_niepoprawne.png[]

.Poprawne rozmieszczenie elementów w modelu - granice się nie przecinają
image::img/model_rozmieszczenie_detali_poprawne.png[]

Wartość tego podejścia stanie się jasny w rozdziale xref:#prezentacja[Przedstaw model na wydruku].
...
:tip-caption: porada
TIP: To jest sytuacja, gdzie zaleta xref:#model-rozmieszczenie-detali[rozmieszczenia detali w modelu tak żeby ich obrysy się nie przecinały] staje się widoczna.
