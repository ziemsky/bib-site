...
* jedno z "menów" wymienionych na liście na górze okna (np. "_Edit_" na ilustracji powyżej),
...
Zauważ, że rozmieszczenie paneli i ich "zakładek" różni się między dwiema ilustracjami. Wynika to z tego, że to rozmieszczenie można zmieniać, tak aby dostęp do nich był najwygodniejszy:
...
Pdobnie, łapiąc i przesuwając poziomo zorientowane tytuły paneli, możesz zmienić lokację tych paneli.
...
Na dysku Twojgo komputera stwórz katalog "_qcad biblioteka_" a w nim katalog "_ramki_" (np. w katalogu "_Dokumenty_").

Ściągnij następujące pliki, jeden po drugim, do tego nowo utworzonego katalogu "_ramki_":

* xref:drawings/ramki/A0 pion.dxf[A0 pion.dxf]
* xref:drawings/ramki/A0 poziom.dxf[A0 poziom.dxf]
* xref:drawings/ramki/A1 pion.dxf[A1 pion.dxf]
* xref:drawings/ramki/A1 poziom.dxf[A1 poziom.dxf]
* xref:drawings/ramki/A2 pion.dxf[A2 pion.dxf]
* xref:drawings/ramki/A2 poziom.dxf[A2 poziom.dxf]
* xref:drawings/ramki/A3 pion.dxf[A3 pion.dxf]
* xref:drawings/ramki/A3 poziom.dxf[A3 poziom.dxf]
* xref:drawings/ramki/A4 pion.dxf[A4 pion.dxf]
* xref:drawings/ramki/A4 poziom.dxf[A4 poziom.dxf]
* xref:drawings/ramki/A4 poziom obrocona.dxf[A4 poziom obrocona.dxf]
* xref:drawings/ramki/tabliczka.dxf[tabliczka.dxf]

Dodaj je do biblioteki QCAD'a:
...
=== Ściągnij bazowy plik
Ściągnij jeden z następujących plików na swój komputer, zapisz go w katalogu Twojego projektu i zmień jego nazwę:

* xref:drawings/rysunek qcad - wymiary w milimetrach.dxf[rysunek qcad - wymiary w milimetrach.dxf] - dla rysunków stali
* xref:drawings/rysunek qcad - wymiary w centymetrach.dxf[rysunek qcad - wymiary w centymetrach.dxf] - dla rysunków obiektów murowanych i drewnianych

=== Warstwy
Ściągnięte pliki zawierają kilka użytecznych, już skonfigurowanych warstw. Uzyj ich dla uproszczenia rysowania:

* `konstrukcja` - tu rysuj swój model - elementy stalowe, więźby, mury, etc.
* `widoki` - tu umieszczaj widoki elementów dla wydruku (jak opisane w xref:#nowy-widok[])
* `wymiary opisy (model)` - tu umieszczaj pomocnicze opisy i wymiary, które powinny być widoczne tylko w modelu, a które nie powinny być drukowane na papierze. Innymi słowy, są to elementy, które służą Tobie, jako rysownikowi, ale które nie powinny być uwidocznione czytelnikowi rysunku.
* `wymiary opisy (papier)` - tu umieszczaj opisy i wymiary, które powinny być drukowane na papierze.
...
* umieszczać je na xref:#niedrukowalna-warstwa[niedrukowalnej warstwie] `wymiary opisy (model)`.,
...
Otwórz: `_Edit > *Drawing preferences* > Printing > Page_`. Tam:
...
[WARNING]
--
Nie pomyl ikony, i *nie kliknij* omyłkowo _Auto center_ (image:img/auto_center.png[]). To narzędzie automatycznie przesuwa elementy już umieszczone na stronie do jej centrum, czego odwrócenie nie zawsze jest łatwe.
...
W panelu _Libray browser_ po prawej stronie ekranu (wspomniany w sekcji xref:#kluczowe-panele[]):
...
* w panelu poniżej `LMBx2` ramkę o rozmiarze i orientacji odpowiadających rozmiarowi i orientacji stronie (nadanych w kroku xref:ustaw-rozmiar-orientacje-strony[]),
* wybierz `_Snap > Grid_`,
* kliknij w rogu, w miejscu przecięcia się szarych, wąskich krawędzi białego obszaru (punkt przedstawiony czerwonym celownikiem na poniższej ilustracji),

image::img/anchor_frame.png[align=center]

* wciśnij klawisz _Esc_, żeby odłożyć narzędzie,
* pomniejsz/wycentruj widok, tak aby zobaczyć całą stronę (image:img/auto_zoom.png[]),
* zaznacz ramkę (`LMB`),
* wybierz `_Block > Explode_`.

[NOTE]
Wklejona ramka automatycznie dodaje nową warstwę o nazwie "_ramka tabliczka_". Warstwy tej nie usuwaj, bo stracisz ramki z tego i pozostałych stron w bieżącym pliku rysunku.

[NOTE]
Ramka ta dodaje również blok o nazwie takiej, jaką wybrałeś z biblioteki (np. _A2 poziom_). Ten blok można usunąć, ale tylko jeśli wykonałeś `_Block > Explode_` na ramce, jak opisano powyżej.

=== Wypełnij tabliczkę

Wklejona powyżej ramka zawiera tabliczkę. Aby wypełnić ją właściwą treścią `LMBx2` teksty w indywidualnych polach aby je zmienić. Niektóre z pól zawierają tylko podkreślenia; w tym przypadku `LMBx2` te podkreślenia i zastąp je właściwym tekstem.

[NOTE]
Jeśli nowy tekst wykracza poza objętość pola, zmniejsz go w oknie dialogowym w polu "_Height_" w lewym górnym rogu. Wartość tego pola wyraża się w jednostkach rysunku.

[#nowy-widok]
=== Dodaj widok elementu

Niektóre elementy są w modelu zorientowane w pionie lub poziomie. Takie elementy zwykle będziemy przedstawiać na papierze w tej samej orientacji, w jakiej znajdują się w modelu. Przykładem są słupy, poziome belki lub blachy czołowe:
...
image::img/info_angle_dialogue.png[align=center]
...
// todo common operations
Upewnij się, że warstwa "wymiary opisy (papier)" jest aktywna w _Layer list_. Jeśli nie jest to `LMB` jej nazwę w panelu _Layer list_.
