...
* `kontury` - tu rysuj swój model - elementy stalowe, więźby, mury, etc.
* `widoki (papier)` - tu umieszczaj widoki elementów dla wydruku (jak opisane w xref:#nowy-widok[])
* `wymiary opisy (model)` - tu umieszczaj pomocnicze opisy i wymiary, które powinny być widoczne tylko w modelu, a które nie powinny być drukowane na papierze.
