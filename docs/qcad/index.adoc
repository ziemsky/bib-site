...
image::img/print_preview_black_and_white.png[]
...
.Auto zoom to page(s)
image::img/presentation_auto_zoom_to_pages.png[]

[WARNING]
Nie pomyl ikony, i nie kliknij _Auto center_ (image:img/auto_center.png[]) przez pomyłkę. To narzędzie automatycznie przesuwa elementy już umieszczone na stronie do jej centrum, zamiast przesuwać _podgląd_ samej strony, bez wpływu na jej zawartość.

=== Dodaj ramkę

Otwórz bibliotekę: zaznacz _View > Library Browser_.

W prawym, górnym rogu okna pojawi się przeglądarka biblioteki (_Library Browser_). Tamże:

* wybierz zakładkę _File system_,
* w rozwijalnym menu pod zakładką wybierz _ramki_,
* w panelu poniżej zaznacz _ramki_,
* powiększ lewy dolny róg białej strony na ekranie,
* w panelu poniżej `LMBx2` wybierz ramkę o rozmiarze i orientacji takich jakie nadałeś stronie (w kroku xref:ustaw-rozmiar-orientacje-strony[]),
* wybierz _Snap > Grid_,
