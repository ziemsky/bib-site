...
* `widoki (papier)` - tu umieszczaj widoki elementów dla wydruku (jak opisane w xref:#nowy-widok[])
...
[NOTE]
Wklejona ramka automatycznie dodaje nową warstwę o nazwie "_ramka tabliczka (papier)_". Warstwy tej nie usuwaj, bo stracisz ramki z tego i pozostałych stron w bieżącym pliku rysunku.

[NOTE]
Ramka ta dodaje również blok o nazwie takiej jaką wybrałeś z biblioteki (np. _A2 poziom_). Ten blok można usunąć.
...
===== Wejdź na warstwę "widoki (papier)"
// todo common operations

Upewnij się, że warstwa "widoki (papier)" jest aktywna w _Layer list_. Jeśli nie jest, to `LMB` jej nazwę w panelu _Layer list_.
